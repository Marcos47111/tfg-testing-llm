...
\begin{enumerate}
    \item \textbf{Fase 1: Doble anotación independiente y a ciegas}: Dos evaluadores humanos ($E_1$ y $E_2$) califican de forma totalmente desacoplada e independiente la totalidad de las 126 respuestas generadas ($42 \text{ casos} \times 3 \text{ perfiles} \times 7 \text{ dimensiones} = 882 \text{ juicios pareados}$), sin acceso a las puntuaciones del otro evaluador ni a la etiqueta identificativa del perfil conversacional emisor. Las anotaciones independientes de esta fase se preservan de forma verificable a partir del estado histórico conservado en el repositorio Git en \path{data/evaluaciones/raw_independientes/}, junto con el registro formal de sus cuatro discrepancias iniciales (\path{data/evaluaciones/adjudicaciones_fase1_independiente.csv}).
    \item \textbf{Fase 2: Revisión, calibración y consolidación del Gold Standard}: Tras la doble anotación independiente, se procede a un proceso sistemático de revisión y calibración técnica frente a las evidencias textuales de las respuestas, las rúbricas formales y las soluciones canónicas. El Evaluador 1 (autor del trabajo y diseñador del instrumento), en contraste con el Evaluador 2, consolida un único conjunto de referencia canónico (\textit{Gold Standard}) formalizado en \path{data/evaluaciones/evaluacion_gold_standard.json} y respaldado por el registro auditable de decisiones en \path{data/evaluaciones/adjudicaciones_gold_standard.csv}, base sobre la que se computan los indicadores analíticos oficiales ($IQE$, $CFR$, $HR$) y se evalúa el juez automático.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Aislamiento e independencia de datos}: El juez automático nunca recibe las puntuaciones ni las justificaciones cualitativas de $E_1$ o $E_2$ durante el proceso de juicio. Las evaluaciones humanas independientes originales se preservan en \path{data/evaluaciones/raw_independientes/}, separadas de los datasets revisados de referencia y de las evaluaciones del juez automático, y los datos del juez se custodian en un directorio independiente (\path{data/evaluaciones/llm_judge/}), distinguiendo entre trazas de inferencia sin procesar (\textit{raw}) y datasets normalizados.
